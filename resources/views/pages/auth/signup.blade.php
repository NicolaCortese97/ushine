    ...
    <div class="auth-background flex h-auto min-h-screen items-center justify-center overflow-x-hidden bg-cover bg-center bg-no-repeat py-10">
        <div class="relative flex items-center justify-center px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-base-100 shadow-base-300/20 z-1 w-full space-y-6 rounded-xl p-6 shadow-md sm:max-w-md lg:p-8">
                ...
                <div>
                    <h3 class="text-base-content mb-1.5 text-2xl font-semibold">Registrati</h3>
                    <p class="text-base-content/80">Crea il tuo account e brilla con il tuo talento.</p>
                </div>
                <div class="space-y-4">
                    <form class="mb-4 space-y-4" action="{{ route('register') }}" method="POST">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                <label class="label-text" for="cognome">Cognome*</label>
                                <input type="text" name="cognome" placeholder="Il tuo cognome" class="input @error('cognome') input-error @enderror" id="cognome" value="{{ old('cognome') }}" required />
                                @error('cognome')
                                <span class="text-error text-sm mt-1">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div>
                            <label class="label-text" for="userEmail">Indirizzo Email*</label>
                            <input type="email" name="email" placeholder="esempio@email.com" class="input @error('email') input-error @enderror" id="userEmail" value="{{ old('email') }}" required />
                            @error('email')
                            <span class="text-error text-sm mt-1">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div>
                            <label class="label-text" for="tipo_utente">Chi sei?*</label>
                            <select name="tipo_utente" class="select @error('tipo_utente') select-error @enderror w-full" id="tipo_utente" required>
                                <option value="" disabled selected>Seleziona il tuo ruolo</option>
                                <option value="Talent" {{ old('tipo_utente') == 'Talent' ? 'selected' : '' }}>Talent (Mostra il tuo talento)</option>
                                <option value="Sponsor" {{ old('tipo_utente') == 'Sponsor' ? 'selected' : '' }}>Sponsor (Supporta i talenti)</option>
                                <option value="Visitatore" {{ old('tipo_utente') == 'Visitatore' ? 'selected' : '' }}>Visitatore</option>
                            </select>
                            @error('tipo_utente')
                            <span class="text-error text-sm mt-1">{{ $message }}</span>
                            @enderror
                        ...
                        <div>
                            <label class="label-text" for="userConfirmPassword">Conferma Password*</label>
                            <div class="input">
                        ...
                        <button class="btn btn-lg btn-primary btn-gradient btn-block" type="submit">Registrati</button>
                    </form>
                    <p class="text-base-content/80 mb-4 text-center">
                        Hai già un account?
                        <a href="{{ route('login') }}" class="link link-animated link-primary font-normal">Accedi invece</a>
                    </p>
                </div>
            </div>
        </div>
